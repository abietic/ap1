...
-- local seq_val = tonumber(ARGV[1])
-- local step_val = tonumber(ARGV[2])
local seq_val = ARGV[1]
local step_val = ARGV[2]
...
if tonumber(redis.call('EXISTS', seq_val_key)) ~= 0 and tonumber(redis.call('EXISTS', seq_step_key)) ~= 0 then
    ...
    redis.call('INCRBY', seq_val_key, redis.call('GET', seq_step_key))
    return seq
else
    redis.call('SET', seq_val_key, seq_val + step_val)
    redis.call('SET', seq_step_key, step_val)
    return seq_val
end